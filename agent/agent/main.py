...
from elara_wrapper import elara_auth_router, GeneratePostRequestBody, add_elara_middleware
...
agent = Agent(model=get_model("NousResearch/Hermes-3-Llama-3.1-8B"),
...
async def root(body: GeneratePostRequestBody):
    try:
        ...
        raise HTTPException(status_code=HTTPStatus.INTERNAL_SERVER_ERROR, detail=f"An error occurred while running the agent: {e}")
